...
// Generated by metaharness — the `curso-interactivo` CLI entry point.
//
// This is plain ESM JavaScript on purpose: it runs as-is via `npx curso-interactivo`
...
// "bin" + "files" fields in package.json), so `npx curso-interactivo` works the moment
// `npm install` has resolved @metaharness/kernel + @metaharness/host-hermes.
...
/** `curso-interactivo init` — boot the kernel + host adapter and report status. */
async function init() {
...
/** `curso-interactivo doctor` — verify the install end-to-end (kernel + host resolve). */
